...
sever = Sever(0, config.SERVER_ADDR, config.SERVER_PORT, config.model_name)
...
if offload:
	...
	agent.policy.load_state_dict(torch.load('./mobilenetv3_basic.pth'))
...
for r in range(config.R):
	...
	with open('MobileNetV3.pkl','wb') as f:
				pickle.dump(res,f)
