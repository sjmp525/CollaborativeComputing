...
class Trainer():
    ...
    def train(self) -> Dict:
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            WARMUP_ROUNDS = 150
            if hasattr(self.server, 'clustering_interval') and epoch > WARMUP_ROUNDS and epoch > 0 and epoch % self.server.clustering_interval == 0:
                self.server._update_clusters()
            ...
            is_fedacg = self.args.server.get('FedACG', False)
            is_clustered_fedacg = 'Clustered' in self.server.__class__.__name__

            if is_fedacg and not is_clustered_fedacg:
                assert(self.args.server.momentum > 0)
                lookahead_model = copy.deepcopy(self.server.FedACG_lookahead(copy.deepcopy(self.model)))
                model_to_send_dict = lookahead_model.state_dict()
            ...
            for i, client_idx in enumerate(selected_client_ids):
                ...
                if is_clustered_fedacg:
                    lookahead_model = self.server.FedACG_lookahead(copy.deepcopy(self.model), client_idx)
                    model_to_send_dict = lookahead_model.state_dict()
                elif is_fedacg:
                    pass 
                else:
                    model_to_send_dict = self.model.state_dict()

                task_queue_input = {
                    'state_dict': model_to_send_dict,
                ...
                if self.args.multiprocessing:
                    ...
                    baseline_state_dict = task_queue_input['state_dict']

                    for param_key in local_state_dict:
                        local_weights[param_key].append(local_state_dict[param_key])
                        local_deltas[param_key].append(local_state_dict[param_key] - baseline_state_dict[param_key])
            ...
            if self.args.multiprocessing:
                ...
                for client_idx in selected_client_ids:
                    if is_clustered_fedacg:
                        lookahead_model = self.server.FedACG_lookahead(copy.deepcopy(self.model), client_idx)
                        baseline_dicts.append(lookahead_model.state_dict())
                    elif is_fedacg:
                        baseline_dicts.append(model_to_send_dict)
                    else:
                        baseline_dicts.append(self.model.state_dict())
                ...
                for i in range(len(selected_client_ids)):
                    ...
                    baseline_state_dict = baseline_dicts[i]
